			...
			$(t.wrap).on("click",".sc-imgctl",function(e){
				...
				if(tar.find(".img-gif")[0].getAttribute("data-original")){
					...
					if(tar&&e.target.classList.contains(t.gifClass)&&!e.target.classList.contains("on")){
						...
						tar.find("img")[0].onload = function(){
...
;(function(){
	...
	$(".img-gif,.img-jpg").each(function(i,dom){
		var this_url = $(this).attr("data-original")
		showImageData.imgList.push(this_url)

		$(this).bind("click",function(){
			showImageData.index = i;
			image_interface(JSON.stringify(showImageData))	
		})
	})
